...
def hook_for(n):
    # лёгкий per-country крючок (без цифр/имён) — чтобы карточка не пустовала
    if n >= 4:
        return "Несколько вариантов доступа"
    if n >= 2:
        return "Есть рабочие варианты"
    return "Есть прямой доступ"
...
def main():
    ...
    # cluster -> ISO -> count ; cluster -> #глобальных партнёров
    ...
    for cl in CLUSTER_ORDER:
        ...
        for iso, n in sorted(isos.items(), key=lambda x: -x[1]):
            countries[iso] = {
                ...
                "hook": hook_for(n),           # лёгкий крючок (без цифр/имён)
                "fact": facts.get(iso),        # факт по базовой стране (softened, или null)
                "n": n,                        # внутр. сорт; в UI число НЕ показываем
            }
        clusters[cl] = {
            "emoji": emoji,
            "label": label,
    ...
    # сводка
    print("✓ map_data.json записан:", OUT)
    print(f"  кластеров: {len(clusters)}  ·  фактов(база-21): {len(facts)}")
    for cl in out["meta"]["cluster_order"]:
        c = clusters[cl]
        g = "  +🌐глоб" if c["global"] else ""
        print(f"  {cl:10} {len(c['countries']):3} стран{g}")
